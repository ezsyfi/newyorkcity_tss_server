    ...
    T: serde::ser::Serialize,
{
    match db {
        DB::Local(rocksdb_client) => {
            ...
            info!("Insert {} of ({}) into db SUCCESS", name.to_string(), identifier);
            Ok(())
        }
        DB::ConnError(msg) => {
    ...
    T: serde::de::DeserializeOwned,
{
    match db {
        DB::Local(rocksdb_client) => {
            ...
            match vec_option {
                Some(vec) => {
                    info!("Get {} of ({}) from db SUCCESS", name.to_string(), identifier);
                    Ok(serde_json::from_slice(&vec)?)
                },
                None => {
                    error!("Get {} of ({}) from db FAILED", name.to_string(), identifier);
                    Ok(None)
                },
            }
        }
